...
/**
 * Return a new `Pattern` with the given `str`.
 *
 * @param {String} str
 * @return {Pattern}
 * @api public
...
exports.pattern = function(str){
  utils.assertType(str, nodes.String, 'pattern');
  return new nodes.Pattern(str);
};

(exports['pattern-parse'] = function(pattern, expr){
  var ret = new nodes.Expression;
  pattern = utils.unwrap(pattern).first;
  expr = utils.unwrap(expr);
  utils.assertType(pattern, nodes.Pattern, 'pattern');
  utils.assertType(expr, nodes.Expression, 'expression');
  return ret;
}).raw = true;
